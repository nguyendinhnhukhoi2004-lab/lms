...
// Notice the \ufeff to catch potential BOM added by PS, and we replace up to -- LỚP HỌC
const regex = /(?:\\ufeff|\\uFEFF)?(?:\\xEF\\xBB\\xBF)?(?:﻿)?INSERT INTO subjects \(id, name, grade, (?:category, )?description\) VALUES[\s\S]*?(?=-- LỚP HỌC)/;
seed = seed.replace(regex, insertText);

// Fix Giáo dục công dân -> Giáo dục kinh tế và pháp luật in head_subjects
seed = seed.replace(/\$q\$Giáo dục công dân\$q\$/g, '$q$Giáo dục kinh tế và pháp luật$q$');
seed = seed.replace(/\$q\$Gi├ío dß╗Ñc c├┤ng d├ón\$q\$/g, '$q$Giáo dục kinh tế và pháp luật$q$');
